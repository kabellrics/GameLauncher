...
namespace GameLauncher.Connector
{
    public class GameLauncherClient
    {
        ...
        public async Task<IEnumerable<MetadataGenre>> GetMetadataGenresAsync()
        {
            var request = new RestRequest("/api/MetadataGenre", Method.Get);
            var response = await _client.ExecuteAsync(request);

            if (response.IsSuccessful)
            {
                Console.WriteLine("Items: " + response.Content);
                return JsonConvert.DeserializeObject<IEnumerable<MetadataGenre>>(response.Content);
            }
            else
            {
                Console.WriteLine("Error: " + response.ErrorMessage);
                return new List<MetadataGenre>();
            }
        }
        ...
        public async Task<IEnumerable<MetadataGenre>> GetMetadataGenresByItemAsync(Guid id)
        {
            var request = new RestRequest($"/api/MetadataGenre/ByItem/{id}", Method.Get);
            var response = await _client.ExecuteAsync(request);
